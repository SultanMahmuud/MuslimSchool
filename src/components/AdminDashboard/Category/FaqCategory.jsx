...
const FAQCategory = ({ featured, texteld, featured__vis, button_form }) => {
 ...
 useEffect(() => {
  fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/category`)
    ...
    .then((data) => {
      if (data?.data?.length > 0) {
        setCategory(data.data[0].FAQ); // directly save only courses
      }
    });
}, [categoryItem]);


  // Submit new FAQ category
  const onSubmit = (data) => {
    const newData = { ...data };
    axios
      .put(`${BASE_URL}/category/FAQ/68af1a2bda8917c50135cebc`, newData)
      .then(() => {
        reset();
  ...
  const handleDeleteCours = (id) => {
    ...
    axios
      .put(`${BASE_URL}/category/FAQDelete/${id}`, data)
      .then((response) => {
